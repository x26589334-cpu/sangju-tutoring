...
- 검색엔진 소유확인: 네이버 `ab9dff4ad604af4c3fca70b1a5e01bf5b7dfe47b` · 구글 `DFiRsBw-V0UnyZDM7q7x4rfGXbXcWpDIbDg6ki_2pGo` — 홈 `<head>` 에 생성기가 넣는다(`regions.js` 의 `verify`). **`index.html` 에 손으로 넣지 말 것** — 재생성하면 사라진다. 코드를 바꾸려면 `verify` 를 고치고 재생성.
